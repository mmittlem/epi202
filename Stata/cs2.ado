* cs2 Command for Count Data (EPI 201)
* Modified October 4, 2018 by Joy Shi

program define cs2, rclass sort
	version 6, missing
	syntax varlist(min=2 max=2) [if] [in] [fw] [, Binomial(varname) /*
	    */ BY(string) noHom noHEt noCrude Exact EStandard Fast IStandard invvar /*
	...
	local stdflg=("`standard'"!="")+("`estandard'"!="")+("`istandard'"!="")+("`invvar'"!="")
	...
	* checks that weight and binomial are not both present
	...
	if  "`by'"~="" {
		...
		if `count' > 1 {
			...
			*cs2 `cas' `ex' `if' `in' `w' `myopt'
			local myopt ", `invvar' `estandard' `istandard' `pool' `or' `tb' `woolf' `exact' by(`myby',`missing') `rd' level(`level') `crude' `hom' `stdopt'"
			cs2 `cas' `ex' `if' `in' `w' `myopt'
			ret add
			exit
        	}
...
* Call dispcs2
		...
		begin options 
			...
			*/	hdr2("`hdr2'") paren("`paren'") `crude'
			...
			ret add    /* add return values from dispcs2 */
		...
		* csi `a' `b' `c' `d', level(`level') `col' `woolf'
		...
		local n0=`b'+`d'
		local t=`n1'+`n0'
		local ivw=(`n1')^3*(`n0')^3/((`n0')^3*`a'*`c'+(`n1')^3*`b'*`d')
