* ir2 command for Person time data (EPI202)
* Modified October 27 2018 by Geetha Iyer

program define ir2, rclass sort
	version 6, missing
	syntax varlist(min=3 max=3) [if] [in] [fw] [, BY(string) /*
		*/ noCrude noHom noHEt EStandard Fast IRD IStandard invvar /*
	...
	local stdflg=("`standar'"!="")+("`estanda'"!="")+("`istanda'"!="")+("`invvar'"!="")
	if `stdflg'>1 { 
...
local dfm1 = `cdf'
		...
		if `n1'>=. { local n1 0 } 
		if `n0'>=. { local n0 0 } 
		local ivw=((`n1')^2*(`n0')^2)/((`n0')^2*`a'+(`n1')^2*`b')
